...
export class RomMatcher {
    ...
    constructor(roms) {
        ...
        for (const [system, aliases] of Object.entries(systemAliases)) {
            const systemRoms = this.normalizedRoms.filter(rom => aliases.some(alias => rom.systemFolders.has(alias)));
    ...
    findMatch(targetName, systemContext) {
        // Check cache first
        ...
        if (systemRoms) {
            ...
            const systemMatch = exactMatches.find(rom => systemRoms.includes(rom) &&
        ...
        if (results.length > 0 && systemRoms) {
            ...
            const bestInSystem = results.find(r => {
                ...
                if (!rom || !systemRoms.includes(rom))
                    return false;
                // Threshold check
                if (r.score >= 0.8)
                    return false;
