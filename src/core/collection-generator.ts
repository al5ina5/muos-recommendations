...
import { Settings, ICollectionFormatter, SpruceOSCollection } from '../types.js';
import { RomMatcher, RomFile } from '../utils/matcher.js';
import type { Collection } from '../data/recommendations.js';
...
export async function generateCollections(
    ...
    matcher: RomMatcher,
    formatter: ICollectionFormatter,
    settings: Settings
): Promise<void> {
...
async function generateSpruceOSCollections(
    collections: Collection[],
    matcher: RomMatcher,
    formatter: ICollectionFormatter,
    settings: Settings
): Promise<void> {
    const spruceFormatter = formatter as any; // SpruceOSFormatter
    const allCollections: SpruceOSCollection[] = [];
    let totalFound = 0;
    let totalGames = 0;
...
async function generateMuOSCollections(
    collections: Collection[],
    matcher: RomMatcher,
    formatter: ICollectionFormatter,
    settings: Settings
    ...
    for (const collection of collections) {
        ...
        for (const { game, match, index: i } of gameMatches) {
            if (!match && settings.missingHandle === 'omit') continue;
